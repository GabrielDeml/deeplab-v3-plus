...
def eval_input_fn(image_filenames, label_filenames=None, batch_size=1):
  ...
  """
  # dataset = tf.data.Dataset.from_tensor_slices(image_filenames[0])
  # # dataset = dataset.map(lambda x: x)
  # iterator = dataset.make_one_shot_iterator()
  # images = iterator.get_next()
  # print(image_filenames[0])
  # image_string = tf.read_file(image_filenames[0])
  ...
  temparray.append(image_filenames[0])

  cap = cv2.VideoCapture(0)

  dataset = tf.data.Dataset.from_tensor_slices(temparray)
  # if label_filenames is None:
  #   dataset = dataset.map(lambda x: _parse_function(x, False))
  # else:
  #   dataset = dataset.map(lambda x, y: _parse_function((x, y), True))

  ret, image_np = cap.read() #/ 255
  img2 = cv2.resize(image_np, dsize=(513, 513), interpolation=cv2.INTER_CUBIC)

  t = tf.convert_to_tensor(img2, dtype=tf.float32)
  # np_image_data = np.asarray(img2)
  # # maybe insert float convertion here - see edit remark!
  # np_final = np.expand_dims(np_image_data, axis=0)


  # img3 = tf.sparse_to_dense(np_final, []
  # print(str(image_np))
  # image_np_expanded = np.expand_dims(image_np, axis=0)
  # label = tf.to_int32(tf.image.convert_image_dtype(image_np_expanded, dtype=tf.uint8))
  # label.set_shape([None, None, 1])
